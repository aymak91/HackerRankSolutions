// There are n balls placed on a 1-dimensional axis with each of them moving with the same non-zero speed. direction[i] represents the
//  direction in which the ith ball is moving, with -1 meaning that it is moving to the left and 1 meaning it is moving to the right. 
//  The strength of the ith ball is described by strength[i]. If 2 balls collide, the one with the higher strength destroys the smaller one.
//  If both have the same strength, both are destroyed. Return a list of the indices of the balls that remain after all the collisions
...
// Space: O(n) where in is the number of balls
// Similar problem: https://leetcode.com/problems/asteroid-collision/